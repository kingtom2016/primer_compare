...
alpha<-function (t_otu, tree = NULL, base = exp(1)) {
    library(picante)
    library(vegan)
    x <- t_otu
    est <- estimateR(x)
...
physeq<-physeq_v4
tmp1<-alpha(t_otu = t(otu_table(physeq)),phy_tree(physeq)) ; tmp1$primer<-"V4"; tmp1$sample_name<-rownames(tmp1)
tmp1$hill_1<- hillR::hill_taxa(t(otu_table(physeq)), q = 1)
tmp1$hill_2<- hillR::hill_taxa(t(otu_table(physeq)), q = 2)
physeq<-physeq_v5v7
tmp2<-alpha(t_otu = t(otu_table(physeq)),phy_tree(physeq));tmp2$primer<-"V5-V7";tmp2$sample_name<-rownames(tmp2)
